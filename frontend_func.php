...
function createDatePickerDefined($elemID, $month, $day, $year, $dayAddons = 0, $class = '', $widthInfo = '100,45,60')
{
	//$day += $dayAddons;
	...
	$selectMonth = '<select id="'.$elemID.'_month" name="'.$elemID.'_month" class="'.$class.'" style="width: '.$width[0].'px;">';
	$selectMonth .= '<option value="0" selected></option>';
	for ($i = 1; $i <= 12; $i++)
	{
		if ($i == $month)
		{
			$selectMonth .= '<option value="'.$i.'" selected>'.ucwords(loadMonther($i)).'</option>';
		}
		else
		{
			$selectMonth .= '<option value="'.$i.'">'.ucwords(loadMonther($i)).'</option>';
		}
	}
	$selectMonth .= '</select>';
	
	$selectDay = '<select id="'.$elemID.'_day" name="'.$elemID.'_day" class="'.$class.'" style="width: '.$width[1].'px;">';
	$selectDay .= '<option value="0" selected></option>';
	for ($i = 1; $i <= 31; $i++)
	{
		if ($i == $day)
		{
	...
	$maxYr = date("Y") + 5;
	$minYr = 1900;
	$selectYear = '<select id="'.$elemID.'_year" name="'.$elemID.'_year" class="'.$class.'" style="width: '.$width[2].'px;">';
	$selectYear .= '<option value="0" selected></option>';
